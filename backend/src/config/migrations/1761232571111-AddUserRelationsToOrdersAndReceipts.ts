...
export class AddUserRelationsToOrdersAndReceipts1761232571111 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (users.length > 0) {
            ...
            await queryRunner.query(`UPDATE "orders" SET "user_id" = $1 WHERE "user_id" IS NULL`, [defaultUserId]);
            await queryRunner.query(`UPDATE "order_items" SET "user_id" = $1 WHERE "user_id" IS NULL`, [defaultUserId]);
            await queryRunner.query(`UPDATE "receipts" SET "user_id" = $1 WHERE "user_id" IS NULL`, [defaultUserId]);
        }
        
        // Теперь делаем колонки NOT NULL
        await queryRunner.query(`ALTER TABLE "orders" ALTER COLUMN "user_id" SET NOT NULL`);
        await queryRunner.query(`ALTER TABLE "order_items" ALTER COLUMN "user_id" SET NOT NULL`);
        await queryRunner.query(`ALTER TABLE "receipts" ALTER COLUMN "user_id" SET NOT NULL`);
