...
export class PdfTestController {
  ...
  async generateTestPdfGet(
    ...
    return this.generateTestPdf({ template, withLogo: withLogo === 'true', language }, res);
  }
  ...
  async previewTemplate(
    ...
    try {
      ...
      const html = await this.templateService.renderTemplate(templateEnum, testData);
  ...
  private getTranslations(language: string) {
    ...
    const fallbackTranslations = {
      en: {
        ...
        terms2: 'Our risk and responsibility ceases as soon as the goods are delivered to the carrier.',
      ...
      ru: {
        ...
        terms2: 'Наш риск и ответственность прекращаются, как только товары доставлены перевозчику.',
      ...
      uk: {
        ...
        terms2: 'Наш ризик та відповідальність припиняються, як тільки товари доставлені перевізнику.',
        forCompany: 'За, {{companyName}}',
  ...
  async generateTestPdf(@Body() body: { template?: string; withLogo?: boolean; language?: string }, @Res() res: Response) {
    try {
      const testData = this.getTestData(body.withLogo !== false, body.language || 'en');

      // Render HTML template
      const templateName = body.template || 'standard';
      const template = templateName as ReceiptTemplate;
      const html = await this.templateService.renderTemplate(template, testData);
