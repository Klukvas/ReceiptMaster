...
export const LegalLayout: React.FC<LegalLayoutProps> = ({
  ...
  return (
    <div className="min-h-screen bg-elevated">
      ...
      >
        <h1 id={titleId} className="text-3xl font-bold text-content mb-2">
          {title}
        </h1>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 flex flex-wrap gap-4 text-sm text-content-tertiary">
          {navLinks.map(({ to, label }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                isActive ? "text-content font-medium" : "hover:text-content"
              }
            >
              {label}
            </NavLink>
